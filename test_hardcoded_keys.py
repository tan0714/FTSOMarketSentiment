...
XAUTH     = os.getenv("W3UP_XAUTH")
AUTH      = os.getenv("W3UP_AUTH")
if not all([SPACE_DID, XAUTH, AUTH]):
    sys.exit("❌ Missing W3UP_SPACE_DID / W3UP_XAUTH / W3UP_AUTH")

STO_HEADERS = {"X-Auth-Secret": XAUTH, "Authorization": AUTH}
...
def upload_car(root, car_cid, car_path, size):
    ...
    resp = requests.post(BRIDGE, headers=STO_HEADERS, json=body).json()
    out  = resp[0]["p"]["out"]
    ...
    if "ok" in out:
        ...
        sys.exit("❌ store/add missing 'ok' in response:\n" + json.dumps(resp, indent=2))
    ...
    r2 = requests.post(BRIDGE, headers=STO_HEADERS, json=reg)
    r2.raise_for_status()
